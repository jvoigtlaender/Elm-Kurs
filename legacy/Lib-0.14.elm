...
import Maybe
import Signal
import Signal (Signal, (<~), sampleOn, constant, foldp, map, map2, merge)
...
elaborateDisplay mr (x1,y1) (x2,y2) f ini upd =
  let x2_ = max (x1 + 1) x2
      ...
      gridMbx = Signal.channel False  -- value here actually irrelevant
      gridCheck = Graphics.Input.checkbox (Signal.send gridMbx)
      restartMbx = Signal.channel ()
      restartButt = case mr of
                      Nothing -> []
                      Just msg -> [ Graphics.Element.spacer 10 10, Graphics.Input.button (Signal.send restartMbx ()) msg ]
      x = x2_ - x1
      y = y2_ - y1
      d = ((-x1' - x2') / 2, (-y1' - y2') / 2)
      f' g p t s =
         Graphics.Element.flow Graphics.Element.up
         [ Graphics.Element.flow Graphics.Element.left <| Text.asText p :: gridCheck g :: restartButt
  ...
  in
   ...
   [ Signal.map
       ...
       (Signal.subscribe gridMbx)
   , Signal.map
       (\_ t _ state -> (0, { state | lastReset <- t, s <- ini }))
       (Signal.subscribe restartMbx)
   ]
   ini upd
